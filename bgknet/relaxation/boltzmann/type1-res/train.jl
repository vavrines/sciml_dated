...
if isNewRun
    ...
    for i in axes(X, 2)
        ...
        X[:, i] .= reduce_distribution(X0[:, :, :, i], vs2.weights)
    end
    ...
    Y = Array{Float64}(undef, vs.nu, length(pdfs))
    for i in axes(Y, 2)
        Y[:, i] .= reduce_distribution(Y0[:, :, :, i], vs2.weights)
    end

    τ = Array{Float64}(undef, 1, size(X, 2))
    for i in axes(τ, 2)
        μ = ref_vhs_vis(kns[i], set.alpha, set.omega)
        w = moments_conserve(X[:, i], vs.u, vs.weights)
        prim = conserve_prim(w, 3)
...
res = sci_train(loss, res.u, Adam(); cb = cb, ad = Optimization.AutoReverseDiff(), iters = 500)
