...
f0 = 0.5 * (1 / π)^1.5 .*
    (exp.(-(vs3.u .- 0.99) .^ 2) .+ 0.7 * exp.(-(vs3.u .+ 0.99) .^ 2)) .*
...
res = sci_train(loss, u, Adam(1e-4); cb = cb, ad = Optimization.AutoReverseDiff(), iters = 500)
res = sci_train(loss, res.u, Adam(1e-4); cb = cb, ad = Optimization.AutoReverseDiff(), iters = 1000)
