...
nu = 48
_nu = nu + 2
_velo = [5.0 / (_nu - 1)^3 * (-_nu+1+2*(i-1))^3 for i = 1:_nu]
velo = (_velo[1:end-1] + _velo[2:end]) ./ 2
du = _velo[2:end] - _velo[1:end-1]
dv, du = meshgrid(du, du)
v, u = meshgrid(velo, velo)
M = maxwellian(u, v, prim0)
weights = zero(u)
for j in axes(weights, 2), i in axes(weights, 1)
    weights[i, j] = du[i] * du[j]
end

sum(M.*weights)

moments_conserve(M, M./2, u, v, weights)


vs = VSpace2D(-5.0, 5.0, 49, -5.0, 5.0, 49, u, v, du, dv, weights)
